...
seoTitle: 'GitHub Copilot Multi-Agent Orchestrator'
...
faq:
  - q: 'What is a multi-agent GitHub Copilot setup?'
    a: 'It is a set of narrow, single-purpose custom agents — developer, tester, security, architecture — each in its own `.agent.md` file with a tight role and least-privilege tools, plus an orchestrator and reusable prompts that sequence them. It is separation of concerns for AI, not a self-organising team.'
  - q: 'Can GitHub Copilot agents work autonomously as a team?'
    a: 'No, not as a closed autonomous loop today. The Copilot coding agent runs multi-step work and custom agents can hand off to each other, but there is no emergent five-agent team that self-organises and ships without you. The human, the hooks, and CI are the real glue between phases.'
  - q: 'How do GitHub Copilot agent handoffs work?'
    a: 'Handoffs are declared in an agent''s YAML frontmatter with a `handoffs` list — a label, a target agent, an optional pre-filled prompt, and a `send` flag. Selecting one switches you to the target agent with context carried over. Handoffs are an IDE-side feature, not supported for the cloud agent on GitHub.com.'
  - q: 'How many Copilot agents should a team start with?'
    a: 'One. Start with a single high-value role — usually a developer or a discovery agent — prove it earns its keep, then add a second only when there is a clear need and an owner for it. Teams that begin with five end up with autonomy theatre: agents nobody uses and nobody is accountable for.'
  - q: 'How do you stop AI agents from contradicting each other?'
    a: 'Give them a shared source of truth. Put your standards in repository instructions and Skills every agent reads, keep each charter narrow so responsibilities do not overlap, and make the seams deterministic — tests and CI decide who is right. When two agents disagree, a human resolves it at the handoff gate.'
---
