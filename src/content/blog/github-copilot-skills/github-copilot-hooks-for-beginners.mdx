...
description: 'A beginner-friendly GitHub Copilot Hooks tutorial: what a Copilot hook is, why you need deterministic guardrails around an AI agent, and how to write your first one in ten minutes.'
...
seoTitle: 'GitHub Copilot Hooks for Beginners: Why and What to Build'
...
faq:
  - q: 'What are GitHub Copilot hooks?'
    a: 'GitHub Copilot hooks are custom shell commands or scripts that run automatically at fixed points during a Copilot agent or Copilot CLI session, such as when the session starts or before the agent runs a tool. They execute outside the AI model, so they are deterministic: they do exactly what you configured, every time, and they can enforce rules the model might otherwise ignore.'
  - q: 'Where do Copilot hooks live in a repo?'
    a: 'In a repository, hooks live in JSON files under `.github/hooks/`, and they apply whenever a Copilot agent runs in that repo. Copilot CLI also reads hooks from `~/.copilot/config.json` for global hooks or `.copilot/config.json` for repo-local ones, under a "hooks" key. Because it is an evolving feature, confirm the exact path against current GitHub docs.'
  - q: "What can a hook do that instructions can't?"
    a: 'Instructions are advisory: they guide a probabilistic model that may or may not follow them. A hook is deterministic code that runs every time, so it can hard-enforce a rule, block a dangerous command before it runs, log every prompt for audit, or run a formatter after every edit. If a check must never be skipped, it belongs in a hook, not in an instruction.'
