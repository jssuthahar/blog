...
seoTitle: 'The Complete AGENTS.md Playbook'
description: 'What belongs in an AGENTS.md file — architecture, security, testing, AI rules — plus what to leave out, common mistakes and a reusable template.'
...
faq:
  - q: 'Can GitHub Copilot generate AGENTS.md automatically?'
    a: "Yes. Type /init in Copilot Chat and it does a good job on what it can see — dependencies, folder structure, patterns already consistent in the code. What it cannot see is your security policy, which of two competing patterns you want going forward, or a rule that lives only in someone's memory. Generate, then edit by hand."
  - q: 'Should I write AGENTS.md by hand or let Copilot generate it?'
    a: "Do both, in that order — generate first, then correct by hand. A blank file wastes the time Copilot would save you on the inferable parts. But shipping its draft as-is is risky, because it describes what your code looks like today, not what you decided it should look like. The human edit is what makes it correct."
...
{/* SCREENSHOT: the screen you were on for this section — save as ./images/what-agents-md-is-and-why-you-need-it-now.png, then replace this line with: ![What AGENTS.md is, and why you need it now](./images/what-agents-md-is-and-why-you-need-it-now.png) */}
...
```markdown
MSDevBuild Eats — a Flutter food delivery app. Flutter 3.x, Dart 3.x.
Web, Android, iOS and tablet from one codebase. Seeded in-memory demo
backend by default; Firebase is the optional second implementation.
```

That one is lifted from my [food delivery app](https://github.com/jssuthahar/food-delivery-app), which I'll keep using for the rest of the examples in this post so you can check every rule against real code.
...
lib/domain is pure Dart and imports no Flutter. Do not introduce a
different architectural pattern for new features, even a well-known one.
```
...
```text
lib/
├── app/             # composition root: DI graph, router, app-wide cubits
├── core/            # shared services, errors, Result, theme, widgets
├── domain/          # pure Dart: entities, repository contracts, use cases
├── data/            # models, data sources, repository implementations
├── features/        # one folder per feature: bloc/ + presentation/
│   ├── cart/
│   ├── orders/
│   └── checkout/
└── main.dart
...
Type `/init` in Copilot Chat and it scans the repo and drafts a file from what it finds. It's genuinely good at the inferable parts: your dependencies, an existing consistent folder structure, a pattern that's already used uniformly across the code. (Same trick works for [Copilot's own custom instructions file](../github-copilot-custom-instructions), which is the smaller, Copilot-specific cousin of this one.)
...
To make this concrete, here's a chunk of the `AGENTS.md` from my [food delivery app](https://github.com/jssuthahar/food-delivery-app). Not an illustration — this is the committed file, and you can open the code behind every rule in it:

```markdown
## Architecture
Clean Architecture, three layers, dependencies point inward only.
lib/domain (pure Dart — no package:flutter import), lib/data,
lib/features/<feature>. Nothing in lib/features imports from lib/data.

## State management
BLoC / Cubit. Not Riverpod, not Provider, not setState.
Blocs are NOT registered in get_it — they resolve their use cases from
it and are created by BlocProvider at the screen that owns them.

## Error Handling
Result<T> from every repository and use case. Never throw across a
repository boundary. Data sources throw AppException; repositories
translate to a Failure through mapErrorToFailure, in exactly one place.

## Business Rules (Hard Constraints)
- Order totals are recomputed from the Cart entity when the order is
  created. Never trust a total posted from the checkout screen.
- PlaceOrder creates the order AND clears the cart.
- OrderStatus owns its own state machine. The partner's advance button,
  the customer timeline and the rider job card read that one definition.
- Adding a dish from a second restaurant emits a pendingConflict state.
  Never silently replace the basket.
```

Notice how specific these are. None of them say "write secure code." Every line is something an agent can actually check itself against — and several of them, like the `get_it` rule, are things no agent could ever infer by reading the code.

That last one is worth pausing on. `get_it` is in the dependency list and blocs are in every feature folder, so the obvious conclusion is to register the blocs too. It's wrong, and an agent finds that out only if you write it down. If you want to see whether the rules held, the app is running as a [live web build](https://jssuthahar.github.io/food-delivery-app/) (`customer@msdevbuild.com` / `demo1234`) and an [Android build](https://appdistribution.firebase.dev/i/00432c5aa60de58b) from the same codebase.
...
{/* SCREENSHOT: the screen you were on for this section — save as ./images/key-takeaways.png, then replace this line with: ![Key takeaways](./images/key-takeaways.png) */}
...
- Once the file is right, the next layer up is [Copilot Skills](../github-copilot-skills-deep-dive) — task-specific knowledge that loads only when the work matches, instead of on every request.
