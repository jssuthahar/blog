...
seoTitle: 'Safely Change Legacy Code with Copilot'
...
faq:
  - q: 'How do I safely refactor legacy code with Copilot?'
    a: 'Pin the current behavior in characterization tests before you touch anything, so any behavior change fails loudly. Then have Copilot generate the change behind a seam using the Strangler Fig pattern, verify parity against those tests, and gate the merge with deterministic hooks and required CI checks.'
  - q: 'What are characterization tests?'
    a: 'Characterization tests capture what code actually does right now — including its bugs — rather than what it should do. You feed a range of inputs, snapshot the real outputs, and assert against those snapshots. Their job is not correctness; it is to make any change in behavior fail loudly.'
  - q: 'Can Copilot write tests for untested legacy code?'
    a: 'Yes, and this is one of its strongest legacy use cases. Copilot can read a method, enumerate representative inputs, and generate golden-master tests that pin current behavior. The catch: it asserts what the code does, not what it should, and can produce tests that pass without exercising the logic.'
  - q: 'What is the Strangler Fig pattern?'
    a: 'The Strangler Fig pattern replaces a legacy system incrementally instead of in one big-bang rewrite. You wrap the old code in a seam, route a small slice of traffic to a new implementation, verify it behaves identically, then expand the slice until the old code is dead. You are never far from a working system.'
  - q: 'Can I trust AI-generated tests?'
    a: 'Trust the ones you have read, not the ones you have counted. AI can generate tests that assert nothing meaningful, encode an existing bug as the correct answer, or skip the edge case that matters. Coverage percentage is not safety. Review for real assertions and curate the cases that carry business risk.'
---
