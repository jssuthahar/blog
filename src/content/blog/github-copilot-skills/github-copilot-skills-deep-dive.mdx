...
description: 'GitHub Copilot Skills explained by an architect — what they are, how they differ from prompts and instructions, and how to build reusable Skills for teams.'
...
seoTitle: 'GitHub Copilot Skills Deep Dive'
...
faq:
  - q: 'What are GitHub Copilot Skills?'
    a: 'GitHub Copilot Skills are reusable capability files — usually a SKILL.md with YAML frontmatter — that teach Copilot how to do a specific job the way your team does it. Copilot reads each Skill''s short description, then loads the full Skill only when a task matches, so knowledge stays reusable without filling the context window.'
...
{/* SCREENSHOT: the real error message you hit — save as ./images/the-real-problem-i-kept-hitting.png, then replace this line with: ![The real problem I kept hitting](./images/the-real-problem-i-kept-hitting.png) */}
...
Different stack, same idea. This one is taken from my [food delivery app](https://github.com/jssuthahar/food-delivery-app), so the folder names and rules below are real rather than illustrative.
...
description: >
  ...
  Enforces the layered structure, BLoC/Cubit state, a repository layer,
  Firebase access only through repositories, and bloc tests.
  Trigger: "new screen", "add feature", "cubit", "bloc", "state".
---

# How we build a Flutter feature

## Rules
- Layers: lib/domain (pure Dart), lib/data, lib/features/{feature}.
- Nothing in lib/features imports from lib/data.
...
- State lives in a Bloc or Cubit, not inside the widget.
- Every bloc has a bloc_test covering loading, data, and failure.

## Workflow
1. Entity + repository interface in lib/domain. Impl in lib/data.
2. Use case in lib/domain/usecases — the bloc asks for one thing.
3. Cubit/Bloc + state class in lib/features/{feature}/bloc.
4. Screen uses BlocProvider + BlocBuilder. No logic in build().
5. bloc_test with a mocked use case. No emulator.
```

The performance rule I always add here: keep `build` methods cheap and push work into the bloc. I have debugged enough janky Flutter screens to know that most "Flutter is slow" complaints are really "we rebuilt an expensive widget tree on every state change" complaints. With BLoC, `buildWhen` is the lever Copilot never reaches for on its own.

You can see all of this running before you read another line of it — the [live web build](https://jssuthahar.github.io/food-delivery-app/) (`customer@msdevbuild.com` / `demo1234`) and an [Android build](https://appdistribution.firebase.dev/i/00432c5aa60de58b) come from the same codebase. [Part 3 of this series](../build-flutter-github-copilot-skill-riverpod-firebase) builds that Flutter Skill line by line.
...
{/* SCREENSHOT: the editor or terminal at this step — save as ./images/how-to-create-effective-copilot-skills.png, then replace this line with: ![How to create effective Copilot Skills](./images/how-to-create-effective-copilot-skills.png) */}
