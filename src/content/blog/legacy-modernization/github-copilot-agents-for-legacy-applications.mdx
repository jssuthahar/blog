...
Developer request
   -> search stale docs
   -> read unfamiliar code for hours
   -> ask the one senior who knows
   -> implement carefully
   -> manually test and hope
```
...
```mermaid
flowchart TD
  U[Developer request] --> M[Main orchestrator agent]
  M --> D[Developer agent]
  M --> T[Tester agent]
  M --> S[Security agent]
  M --> A[Architecture agent]
  D --> V[Final validation]
  T --> V
  S --> V
  A --> V
  V --> R[Reviewed change + tests + notes]
```

The roles collaborate like a real team. The orchestrator reads the request and decides who is involved. The developer agent proposes the change grounded in the Application Knowledge Skill. The tester agent writes unit, integration, and regression tests. The security agent checks authentication, authorization, and sensitive data. The architecture agent checks consistency with existing patterns. Validation pulls it together.
