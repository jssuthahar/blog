...
<Pipeline
  caption="The loop is the point. Nothing moves forward until the current behavior is pinned, and every failure sends you back to the new implementation — never to the old path, which stays working the whole time."
  entry={{ label: 'Change request' }}
  steps={[
    { label: 'Step 1 — Characterization tests', note: 'Pin CURRENT behavior, bugs and all' },
    { gate: 'Human gate', label: 'Do the tests actually exercise the logic?', back: 'Assert-nothing tests → rewrite them before going further' },
    { label: 'Step 2 — Strangler Fig', note: 'Generate the new implementation behind a seam' },
    { label: 'Route one slice to the new path' },
    { gate: 'Check', label: 'Parity against the characterization tests', back: 'Fail → back to the new implementation' },
    { gate: 'CI gate', label: 'Step 3 — Hooks and required checks', back: 'Blocked → back to the new implementation' },
    { label: 'Expand the slice', note: 'Then route again, until the old path carries nothing' },
    { label: 'Old path dead — remove it', accent: true },
  ]}
/>
