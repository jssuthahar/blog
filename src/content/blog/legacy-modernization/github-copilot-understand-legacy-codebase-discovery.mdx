...
description: 'A repeatable method for legacy application discovery: turning the business rules, workflows, and history a decade-old codebase never documented into knowledge GitHub Copilot can actually use.'
...
highlight: 'GitHub Copilot is only as smart as the context you feed it. On a ten-year-old system the code is the symptom, and the real knowledge lives in git history and people''s heads. Discovery extracts that into instructions, an Application Knowledge Skill, and AGENTS.md so Copilot stops guessing. AI can draft the map; a human must verify it, because a confidently wrong knowledge file is worse than none.'
...
```mermaid
flowchart TD
  A[Legacy repo + git history + people] --> P1[Pass 1: Structural map]
  P1 --> G1{Human verify}
  G1 --> P2[Pass 2: Business-rule extraction]
  P2 --> G2{Human verify}
  G2 --> P3[Pass 3: Risk & dependency mapping]
  P3 --> G3{Human verify}
  G3 --> P4[Pass 4: Knowledge from history]
  P4 --> G4{Human verify}
  G4 --> K[Verified knowledge]
  K --> I[copilot-instructions.md]
  K --> PI[path-specific *.instructions.md]
  K --> S[Application Knowledge Skill]
  K --> AM[AGENTS.md]
```
