...
<Callout type="tip" title="A working version of this org chart">
The five role agents below are committed in my [food delivery app](https://github.com/jssuthahar/food-delivery-app), so you can read the real frontmatter rather than a snippet — least-privilege `tools`, `send: false` on every handoff, and a **Must NOT do** line in each charter.

- [`.github/agents/orchestrator.agent.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/agents/orchestrator.agent.md) — routes and sequences; can edit nothing
- [`.github/agents/architecture.agent.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/agents/architecture.agent.md) — which layer, and what else reads it
- [`.github/agents/feature.agent.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/agents/feature.agent.md) — the only agent allowed to edit `lib/`
- [`.github/agents/tester.agent.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/agents/tester.agent.md) — cannot edit `lib/`, cannot approve its own change
- [`.github/agents/security.agent.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/agents/security.agent.md) — read-only, and forbidden from re-reporting two things that are deliberate

The role map is in [`.github/agents/README.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/agents/README.md), the drive script in [`.github/prompts/orchestrate-feature.prompt.md`](https://github.com/jssuthahar/food-delivery-app/blob/main/.github/prompts/orchestrate-feature.prompt.md), and the deterministic gate in [`.github/hooks/`](https://github.com/jssuthahar/food-delivery-app/tree/main/.github/hooks) — a `preToolUse` guard that denies a tool call when `lib/domain` has picked up a Flutter import. It runs the same script as the git hook rather than a second copy of the rule, because two copies drift.
</Callout>
...
<Pipeline
  caption="Not a closed loop. Every phase boundary is a gate — dashed, because it is a condition rather than work — and no agent is allowed to move the flow past one on its own."
  entry={{ label: 'Developer request', note: 'Stated through a reusable prompt' }}
  steps={[
    ...
    { gate: 'Hook + CI gate', label: 'Secrets, dependencies, CodeQL', back: 'Blocked → back to the developer agent' },
    { gate: 'Human gate', label: 'Sign-off — a person owns what ships' },
    { label: 'Pull request / merge', accent: true },
  ]}
/>

The dashed boxes are the gates, and they come in two kinds. Two are humans: approving the plan and signing off on what ships. Two are deterministic machinery: CI and the hooks. The agents advise and act inside their lane; the gates are where the flow is allowed to move forward. That is the whole architecture in one picture: AI does the specialised labour, deterministic checks enforce, and humans own the decisions.
