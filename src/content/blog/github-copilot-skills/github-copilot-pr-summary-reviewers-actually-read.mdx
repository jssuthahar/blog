...
seoTitle: 'GitHub Copilot PR Summary: Descriptions Reviewers Read'
...
faq:
  - q: 'What is a GitHub Copilot PR summary?'
    a: 'It is a pull request description that GitHub Copilot generates by reading your diff. On github.com you click the Copilot icon in the description field and select Summary, and it produces a prose overview plus a bulleted list of changes with the files they touch. It is a first draft of the "what", not a finished description — it does not know why you made the change.'
  - q: 'Can Copilot write my pull request description automatically?'
    a: 'Yes, for the mechanical part. Copilot can draft the summary of what changed from the diff, in the IDE or on github.com (it is not in the Copilot Free tier, and it works best when you start from a blank description). It cannot write the parts that matter most — the reason for the change, the risk, and the rollback plan — because those live in your head and the ticket, not in the diff.'
  - q: 'Why are AI PR summaries often useless?'
    a: 'Because they restate the diff and call it a summary. The reviewer can already read the diff. If the description just says "added CalculateRefund method, modified OrdersController", it has added nothing. A useful summary explains why the change exists, what could break, and how it was verified — and the AI cannot know any of that unless you or your template tell it to look for it.'
  - q: 'How do I make Copilot follow our PR template?'
    a: 'Commit a pull_request_template.md with the sections you want, and add repository custom instructions telling Copilot to fill those exact sections instead of restating the diff. Support for the template is still improving, so treat the generated draft as a starting point and edit the "why" and "risk" sections yourself before you request review.'
  - q: 'Should the AI or the author write the PR description?'
    a: 'Both, in that order. Let the AI draft the summary of changes so you are not staring at a blank box. Then the author owns the intent — the why, the risk, the rollback, the ticket link, and the one line telling the reviewer where to focus. AI drafts, human supplies intent. The author is still accountable for what the description claims.'
---
